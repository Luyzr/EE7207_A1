...
def SOM(dataset, labelset, label_set, j,pn = 16,lr = 1e-5, iternum=0, resume=False, local=True):
    n, d = dataset.shape
    if resume and local:
        cv = np.load('{}_{}_center_vector.npy'.format(pn, label_set))
    elif resume:
        cv = np.load('{}_global_center_vector.npy'.format(pn))
    else:
        ...
        while change > 1e-5:
            ...
            if local:
                ...
                if iternum % 1e3 == 0:
                    ...
                    cv_old = cv.copy()
                    print(change)
                    np.save('{}_global_center_vector.npy'.format(pn), cv)
    return cv

def SOM_c(data, label, m, resume, local=False):
    n, d = data.shape
    label = label.tolist()
    labels = []
    for i in label:
    ...
    if local:
        ...
        for i in range(len(labels)):
            ...
            c.append(SOM(data, label, labels[i], j, m, lr=1e-2, iternum=0, resume=resume, local=local))
            target.append(np.array([labels[i]]*j))
        c = np.concatenate((c[0], c[1]))
        target = np.concatenate((target[0], target[1]))
        return c, target
    else:
        c = SOM(data, label, labels[0], m, m, lr=1e-2, iternum=0, resume=resume, local=local)

...
def SOM(dataset, labelset, label_set, j, lr = 1e-5, iternum=0, resume=False):
    n, d = dataset.shape
    if resume:
        cv = np.load('{}_center_vector.npy'.format(label_set))
    else:
    ...
    while change > 1e-5:
        ...
        if iternum % 1e3 == 0:
            ...
            np.save('{}_center_vector.npy'.format(label_set), cv)
    return cv

def SOM_c(data, label, m):
    n, d = data.shape
    label = label.tolist()
    labels = []
    ...
    for i in range(len(labels)):
        ...
        c.append(SOM(data, label, labels[i], j, lr=1e-2,iternum=0,resume=False))
...
def Method1(train_data, train_label, test_data, getCV=False, pn=16):
...
def Method2(train_data, train_label, test_data, getCV=False, pn=16):
    kf = KFold(5, shuffle=True, random_state=42)
    rr = 1
    if getCV:
        best_pn = 0
...
if __name__ == "__main__":
    ...
    print(SOM_c(train_data, train_label, 15))
